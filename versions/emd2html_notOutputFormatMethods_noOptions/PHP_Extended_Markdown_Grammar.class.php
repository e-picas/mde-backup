...
/**
 *
 * This file was automatically generated with PHP_Extended_Markdown_Builder class on 2012-11-17 11:34:47
 *
	...
	protected function _doAbbreviations_callback($matches) 
	{
		...
		if (isset($this->abbr_desciptions[$abbr])) {
			...
			return $this->hashPart(
				$this->buildAbbreviation( $abbr, array('title'=>$desc))
//				$this->runFormaterMethod('buildAbbreviation', $abbr, array('title'=>$desc))
//				$this->output_formater->buildAbbreviation( $abbr, array('title'=>$desc) )
			);
		} else {
			return $matches[0];
		}
...
//					('.$this->getOption('nested_url_parenthesis_re').')	# href = $4
	...
	protected function _doAnchors_reference_callback($matches) 
	{
		...
		if (isset($this->urls[$link_id])) {
			$attrs=array(
				'href'=>$this->encodeAttribute( $this->urls[$link_id] )
			);
			if ( isset( $this->titles[$link_id] ) ) {
				$attrs['title'] = $this->encodeAttribute( $this->titles[$link_id] );
			}
			...
			$result = $this->hashPart(
				$this->buildAnchor( $link_text, $attrs)
//				$this->runFormaterMethod('buildAnchor', $link_text, $attrs)
//				$this->output_formater->buildAnchor( $link_text, $attrs )
			);
		}
		else {
			$result = $whole_match;
	...
	protected function _doAnchors_inline_callback($matches) 
	{
		...
		$attrs=array( 'href'=>$this->encodeAttribute($url) );
		if (isset($title)) 
		{
			$attrs['title'] = $this->encodeAttribute($title);
		}
		$link_text = $this->runGamut( 'spanGamut', $link_text);
		return $this->hashPart(
			$this->buildAnchor( $link_text, $attrs)
...
// -----------------------------------
	...
	protected function _doAutoLinks_url_callback($matches) 
	{
		...
		return $this->hashPart(
			$this->buildLink( $url, array('href'=>$url))
//			$this->runFormaterMethod('buildLink', $url, array('href'=>$url))
//			$this->output_formater->buildLink( $url, array('href'=>$url) )
		);
	}
	...
	{
		return $this->hashPart(
			$this->buildMailto( $matches[1])
//			$this->runFormaterMethod('buildMailto', $matches[1])
//			$this->output_formater->buildMailto( $matches[1] )
		);
	}
...
// -----------------------------------
	...
	protected function _doBlockQuotes_callback($matches) 
	{
		...
		return "\n". $this->hashBlock(
			$this->buildBlockquote( "\n".$bq."\n", $attrs)
//			$this->runFormaterMethod('buildBlockquote', "\n".$bq."\n", $attrs)
//			$this->output_formater->buildBlockquote( "\n".$bq."\n", $attrs )
		)."\n\n";
	}
...
//				((?=^[ ]{0,'.$this->getOption('tab_width').'}\S)|\Z)	# Lookahead for non-space at line-start, or end of doc
	...
	{
		$codeblock = htmlspecialchars( $this->doOutdent( $matches[1] ), ENT_NOQUOTES);
		# trim leading newlines and trailing newlines
		$codeblock = preg_replace('/\A\n+|\n+\z/', '', $codeblock);
		return "\n\n".$this->hashBlock(
			$this->buildCodeBlock( $codeblock."\n")
//			$this->runFormaterMethod('buildCodeBlock', $codeblock."\n")
//			$this->output_formater->buildCodeBlock( $codeblock."\n" )
		)."\n\n";
	}
	...
	{
		return $this->hashPart(
			$this->buildCodeSpan( htmlspecialchars(trim($code), ENT_NOQUOTES))
//			$this->runFormaterMethod('buildCodeSpan', htmlspecialchars(trim($code), ENT_NOQUOTES))
//			$this->output_formater->buildCodeSpan( htmlspecialchars(trim($code), ENT_NOQUOTES) )
		);
	}
...
// ----------------------------------
	...
	{
		$language  = $matches[2];
		$codeblock = htmlspecialchars($matches[3], ENT_NOQUOTES);
		...
		return "\n\n".$this->hashBlock(
			$this->buildCodeBlock( $codeblock, $attrs)
//			$this->runFormaterMethod('buildCodeBlock', $codeblock, $attrs)
//			$this->output_formater->buildCodeBlock( $codeblock, $attrs )
		)."\n\n";
	}
...
//		$less_than_tab = $this->getOption('tab_width') - 1;
	...
	protected function _doDefinitionsLists_callback($matches) 
	{
		...
		$result = trim( $this->processDefinitionsListItems( $matches[1] ) );
		return $this->hashBlock(
			$this->buildDefinitionList( "\n".$result."\n")
//			$this->runFormaterMethod('buildDefinitionList', "\n".$result."\n")
//			$this->output_formater->buildDefinitionList( "\n".$result."\n" )
		)."\n\n";
	}
...
//		$less_than_tab = $this->getOption('tab_width') - 1;
	...
	protected function _processDefinitionsListItems_callback_dt($matches) 
	{
		...
		foreach ($terms as $term) {
			...
			$text .= "\n".$this->buildDefinitionTerm( $term);
//			$text .= "\n".$this->runFormaterMethod('buildDefinitionTerm', $term);
//			$text .= "\n".$this->output_formater->buildDefinitionTerm( $term );
		}
		return $text . "\n";
	...
	protected function _processDefinitionsListItems_callback_dd($matches) 
	{
		...
		if ($leading_line || preg_match('/\n{2,}/', $def)) {
			...
			$def = "\n".$this->runGamut( 'blockGamut', $this->doOutdent($def."\n\n"))."\n";
		}
		else {
			$def = $this->runGamut( 'spanGamut', $this->doOutdent( rtrim($def) ));
		}
		return "\n".$this->buildDefinitionDescription( $def)."\n";
//		return "\n".$this->runFormaterMethod('buildDefinitionDescription', $def)."\n";
//		return "\n".$this->output_formater->buildDefinitionDescription( $def )."\n";
	}
		...
		while (1) {
			...
			if ($tree_char_em) {
				...
				if ($token_len == 3) {
					...
					$span = $this->buildEmphasis( 'both', $span);
//					$span = $this->runFormaterMethod('buildEmphasis', 'both', $span);
//					$span = $this->output_formater->buildEmphasis( 'both', $span);
					...
					$span = $text_stack[0];
					$span = $this->runGamut( 'spanGamut', $span);
					$span = $this->buildEmphasis( $tag, $span);
//					$span = $this->runFormaterMethod('buildEmphasis', $tag, $span);
//					$span = $this->output_formater->buildEmphasis( $tag, $span);
					$text_stack[0] = $this->hashPart($span);
...
//						$span = $this->output_formater->buildEmphasis( $tag, $span);
				...
				if ($strong) {
					...
					$span = array_shift($text_stack);
					$span = $this->runGamut( 'spanGamut', $span);
					$span = $this->buildEmphasis( 'strong', $span);
//					$span = $this->runFormaterMethod('buildEmphasis', 'strong', $span);
//					$span = $this->output_formater->buildEmphasis( 'strong', $span);
					$text_stack[0] .= $this->hashPart($span);
					$strong = '';
				} else {
					array_unshift($token_stack, $token);
				...
				if ($em) {
					if (strlen($token_stack[0]) == 1) {
						...
						$span = array_shift($text_stack);
						$span = $this->runGamut( 'spanGamut', $span);
						$span = $this->buildEmphasis( 'em', $span);
//						$span = $this->runFormaterMethod('buildEmphasis', 'em', $span);
//						$span = $this->output_formater->buildEmphasis( 'em', $span);
						$text_stack[0] .= $this->hashPart($span);
						$em = '';
					} else {
						$text_stack[0] .= $token;
...
// -----------------------------------
	...
	protected function _doHeaders_callback_setext($matches) 
	{
		...
		$block = $this->buildHeader( 
//		$block = $this->runFormaterMethod('buildHeader', 
//		$block = $this->output_formater->buildHeader(
...
//					('.$this->getOption('nested_url_parenthesis_re').')	# src url = $4
	...
	protected function _doImages_reference_callback($matches) 
	{
		...
		if (isset($this->urls[$link_id])) {
			...
			$result = $this->hashPart(
				$this->buildImage( $attrs)
//				$this->runFormaterMethod('buildImage', $attrs)
//				$this->output_formater->buildImage( $attrs )
			);
		}
		else {
	...
	protected function _doImages_inline_callback($matches) 
	{
		...
		$attrs = array( 
			'src'=>$this->encodeAttribute($url), 
			'alt'=>$this->encodeAttribute($alt_text)
		);
		if (isset($title)) {
			$attrs['title'] = $this->encodeAttribute($title);
		}
...
//		return $this->output_formater->buildListItem( $item )."\n";
	}
...
//		$this->metadata=array();
	...
	protected function _doMetaDataCallback($matches)
	{
		...
		return $this->buildMetaData( array(
//		return $this->runFormaterMethod('buildMetaData', array(
//		return $this->output_formater->buildMetaData(array(
			'name'=>$matches[1],
			'value'=>$matches[2]
		));
	}
...
//		$special_mask_opt = $this->getOption($special_mask);
		$special_mask_opt = isset($this->$special_mask) ? $this->$special_mask : null;
		if (!empty($special_mask_opt)) {
			eval("\$_meta = sprintf('$special_mask_opt', \$meta_name, \$meta_value);");
			return $_meta;
		} else
			return sprintf($this->metadata_mask, $meta_name, $meta_value);
...
//		$less_than_tab = $this->getOption('tab_width') - 1;
	...
	protected function _stripNotes_callback($matches) 
	{
		if (0 != preg_match('/^(<p>)?glossary:/i', $matches[2])) {
			$this->glossaries[ $this->glossary_id_prefix . $matches[1] ] = $this->doOutdent($matches[2]);
//			$this->glossaries[ $this->getOption('glossary_id_prefix') . $matches[1] ] = $this->doOutdent($matches[2]);
		} elseif (0 != preg_match('/^\#(.*)?/i', $matches[1])) {
			$this->citations[ $this->bibliography_id_prefix . substr($matches[1],1) ] = $this->doOutdent($matches[2]);
//			$this->citations[ $this->getOption('bibliography_id_prefix') . substr($matches[1],1) ] = $this->doOutdent($matches[2]);
		} else {
			$this->footnotes[ $this->footnote_id_prefix . $matches[1] ] = $this->doOutdent($matches[2]);
...
//				$backtitle = $this->encodeAttribute( $this->getOption('footnote_backlink_title') );
				$backclass = $this->encodeAttribute( $this->footnote_backlink_class );
				$backtitle = $this->encodeAttribute( $this->footnote_backlink_title );
...
//				$backtitle = $this->encodeAttribute( $this->getOption('glossary_backlink_title') );
				$backclass = $this->encodeAttribute( $this->glossary_backlink_class );
				$backtitle = $this->encodeAttribute( $this->glossary_backlink_title );
...
//				$backtitle = $this->encodeAttribute( $this->getOption('bibliography_backlink_title') );
				$backclass = $this->encodeAttribute( $this->bibliography_backlink_class );
				$backtitle = $this->encodeAttribute( $this->bibliography_backlink_title );
...
//			$text = $this->output_formater->buildListItem( "\n".$note_str."\n", array( 'id'=>$ref_prefix.':'.$encoded_note_id ))."\n\n";
	...
	{
		return 
			$this->buildSpan( trim($matches[1]), array( 'class'=>'glossary name'))
//			$this->runFormaterMethod('buildSpan', trim($matches[1]), array( 'class'=>'glossary name'))
//			$this->output_formater->buildSpan( trim($matches[1]), array( 'class'=>'glossary name'))
			.(isset($matches[3]) ? $this->buildSpan( $matches[2], array(
//			.(isset($matches[3]) ? $this->runFormaterMethod('buildSpan', $matches[2], array(
//			.(isset($matches[3]) ? $this->output_formater->buildSpan( $matches[2], array(
				'class'=>'glossary sort',
				'style'=>'display:none'
			)) : '')
			."\n\n"
	...
	{
		return 
			$this->buildSpan( trim($matches[1]), array( 'class'=>'bibliography name'))
//			$this->runFormaterMethod('buildSpan', trim($matches[1]), array( 'class'=>'bibliography name'))
//			$this->output_formater->buildSpan( trim($matches[1]), array( 'class'=>'bibliography name'))
			."\n\n".$matches[2];
	}
...
//		$citation_node_id = $this->getOption('bibliography_id_prefix') . $matches[1];
		$footnote_node_id = $this->footnote_id_prefix . $matches[1];
		$glossary_node_id = $this->glossary_id_prefix . $matches[1];
		$citation_node_id = $this->bibliography_id_prefix . $matches[1];
...
//					$title = $this->encodeAttribute( $this->getOption('footnote_link_title') );
					$class = $this->encodeAttribute( $this->footnote_link_class );
					$title = $this->encodeAttribute( $this->footnote_link_title );
					$note_str = $this->footnotes[$node_id];
					$back_ref_prefix = 'fnref';
					$ref_prefix = 'fn';
			}
			elseif ($type=='glossary')
...
//					$title = $this->encodeAttribute( $this->getOption('glossary_link_title') );
					$class = $this->encodeAttribute( $this->glossary_link_class );
					$title = $this->encodeAttribute( $this->glossary_link_title );
					$note_str = substr( $this->glossaries[$node_id], strlen('glossary:') );				
					$back_ref_prefix = 'fngref';
					$ref_prefix = 'fng';
			}
			elseif ($type=='citation')
...
//					$title = $this->encodeAttribute( $this->getOption('bibliography_link_title') );
					$class = $this->encodeAttribute( $this->bibliography_link_class );
					$title = $this->encodeAttribute( $this->bibliography_link_title );
					$note_str = $this->citations[$node_id];
					$back_ref_prefix = 'fncref';
					$ref_prefix = 'fnc';
			}
...
//				$this->output_formater->buildSup( 
					$this->buildLink( $num, $attrs), 
//					$this->runFormaterMethod('buildLink', $num, $attrs), 
//					$this->output_formater->buildLink( $num, $attrs), 
...
// -----------------------------------
	...
	public function formParagraphs($text) 
	{
		...
		foreach ($grafs as $key => $value) {
			...
			if ($is_p) {
				$value = $this->buildParagraph( $value);
//				$value = $this->runFormaterMethod('buildParagraph', $value);
//				$value = $this->output_formater->buildParagraph( $value);
			}
			$grafs[$key] = $value;
		}
