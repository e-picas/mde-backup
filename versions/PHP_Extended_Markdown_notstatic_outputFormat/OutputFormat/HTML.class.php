	...
	implements PHP_Extended_Markdown_OutputFormat
{
	 ...
	 */
	public function buildTag( $text, $tag, $attributes=array(), $close=false )
	{
		$attr='';
		if (!empty($attributes))
		{
	 ...
	 */
	public function encodeEmailAddress( $addr, $return_array=false ) 
	{
		$addr = 'mailto:' . $addr;
	...
	{
		list($encoded_address, $displayable_address) = $this->encodeEmailAddress( $address, true );
		$attrs['href'] = $encoded_address;
		return $this->buildLink( $displayable_address, $attrs );
	}
	...
	{
		return $this->buildTag( $text, 'abbr', $attrs );
	}
	...
	{
		return $this->buildTag( $text, 'a', $attrs );
	}
	...
	{
		return $this->buildTag( $text, 'blockquote', $attrs );
	}
	...
	{
		return $this->buildTag( $text, 'code', $attrs );
	}
	...
	{
		return $this->buildTag( $text, 'dl', $attrs );
	}
	...
	{
		return $this->buildTag( $text, 'dt', $attrs );
	}
	...
	{
		return $this->buildTag( $text, 'dd', $attrs );
	}
	...
	public function buildEmphasis( $type, $text, $attrs=array() )
	{
		...
		{
			return $this->buildTag( 
				$this->buildTag( $text, 'strong' ), 
				'em', $attrs );
		} 
		else 
		{
			return $this->buildTag( $text, $type, $attrs );
		}
	}
	...
	{
		return $this->buildTag( $text, 'h'.$level, $attrs );
	}
	...
	{
		return $this->buildTag( '', 'img', $attrs, true );
	}
	...
	{
		return $this->buildTag( $text, 'p', $attrs );
	}
	...
	{
		return $this->buildTag( $text, 'ul', $attrs );
	}
	...
	{
		return $this->buildTag( $text, 'ol', $attrs );
	}
	...
	{
		return $this->buildTag( $text, 'li', $attrs );
	}
	...
	{
		return $this->buildTag( $text, 'sup', $attrs );
	}
	...
	{
		return $this->buildTag( $text, 'span', $attrs );
	}
	...
	{
		return $this->buildTag( $text, 'div', $attrs );
	}
