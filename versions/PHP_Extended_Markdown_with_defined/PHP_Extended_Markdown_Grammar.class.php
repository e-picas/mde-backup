...
//		$block = "<h$level$attr>".$this->runSpanGamut($matches[1])."</h$level>";
		...
		$block = $this->runFormaterMethod('buildHeader', 
			$this->runSpanGamut($matches[1]), $level, $attr);
...
//		$block = "<h$level$attr>".$this->runSpanGamut($matches[2])."</h$level>";
		...
		$block = $this->runFormaterMethod('buildHeader', 
			$this->runSpanGamut($matches[2]), $level, $attr);
	 ...
	 * @formater_overload
	 */
	protected function buildHeader( $text, $level=1, $attr=array() )
	{
...
//		$this->metadata=array();
	...
	{
		$special_mask = 'MARKDOWN_METADATA_MASK_'.strtoupper($meta_name);
		if (defined($special_mask)) {
			eval("\$_meta = sprintf($special_mask, \$meta_name, \$meta_value);");
			return $_meta;
		} else
